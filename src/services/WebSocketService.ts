    ...
    public init(): void {
        ...
        tokenStream.pipe(
            switchMap(({ wsToken }) => {
                ...
                const url = process.env.REACT_APP_WEBSOCKET_SERVER_URL ? `${process.env.REACT_APP_WEBSOCKET_SERVER_URL}?token=${wsToken}` : `${protocol}${window.location.host}/ws/ws?token=${wsToken}`;
